...
class Day4 {
    ...
    private fun test2() {
        ...
        val completePassports = mutableListOf<Passport>()
        passports.forEach {
            if (it.isComplete1())
                completePassports.add(it)
        }

        completePassports.forEach {
            it.assignConditions()
            if (it.isComplete2().not())
                completePassports.remove(it)
        }

        println(completePassports.size)
    }

    private fun getRawPasswords(): List<String> {
        return "$input1$input2$input2b$input3$input4".split("\n\n")
    }
